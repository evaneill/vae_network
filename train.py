...
def score(model,test_data_loader,device,n_samples=5000):
	if n_samples is None:
		n_samples = model.encoder.n_samples
	elif type(n_samples)!=int:
		raise Exception
	...
	with torch.no_grad():
	    for i, [data] in enumerate(test_data_loader):
	        data = data.to(device)
	        output, q_mu, q_log_sigma = model.forward(data,n_samples = n_samples)
	        test_loss += -1*VRBound(0,model,output,q_mu, q_log_sigma,optimize_on='full_lowerbound')

	test_loss /= len(test_data_loader.dataset)*n_samples
	print('====> Test set loss: {:.4f}'.format(test_loss))
	return test_loss # I think I want to divide? Printed output from above seems to be the thing I want
...
if __name__=="__main__":
	...
	parser.add_argument('--test_batch','-testbatch',type=int,default=10) # batch size for evaluation of runtime test error and K=5000 test
